...
import { useCourierBalance } from '@/lib/api/courier'
      ...
      <CardContent>
        {isFetching ? (
          ...
          <p className="text-sm text-muted-foreground">
            Steadfast is not configured on the server, so there is no balance to read.
          </p>
        ) : isError ? (
          <p className="text-sm text-muted-foreground">
            Balance unavailable —{' '}
          ...
          <p className="text-sm text-muted-foreground">
            Not checked yet. Reading it calls Steadfast directly.
          </p>
        ) : (
