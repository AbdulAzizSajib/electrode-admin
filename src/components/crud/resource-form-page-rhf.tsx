...
/**
 * The react-hook-form sibling of `ResourceFormPage`.
 *
 * Eight of the pages moved out of overlays by `replace-admin-modals-with-pages`
 * were written against react-hook-form + zod + the shadcn `Form` primitives, and
 * rewriting all eight onto antd while simultaneously moving them would have made
 * any regression impossible to attribute. So the save semantics are implemented
 * twice — once per form library — over one shared `ResourceFormLayout`, and the
 * two are kept deliberately interchangeable from the caller's side.
 *
 * When the panel finishes migrating to antd, this file is deleted and its
 * callers move to `ResourceFormPage` one at a time. See design.md Decision 1 of
 * `replace-admin-modals-with-pages`.
 *
...
export interface ResourceFormPageRhfProps<
...
export function ResourceFormPageRhf<
  TValues extends FieldValues,
  TRecord,
  TOutput extends FieldValues = TValues,
>({
  ...
  title,
  description,
}: ResourceFormPageRhfProps<TValues, TRecord, TOutput>) {
